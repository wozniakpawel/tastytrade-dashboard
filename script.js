...
function processTradeData(data) {
  ...
  return processedData.map((row) => {
    ...
    const holdingPeriod = Math.max(
      0,
      (row.CLOSE_DATE - row.OPEN_DATE) / (1000 * 60 * 60 * 24)
    );
    ...
    const roi =
      row.NO_WS_COST !== 0
        ? (row.TOTAL_GAINLOSS / Math.abs(row.NO_WS_COST)) * 100
        : 0;
...
function createPlots(data) {
  ...
  createPlot(
    {
      ...
      line: { shape: "hv" },
      hovertemplate:
        "<b>Date</b>: %{x|%Y-%m-%d}<br><b>P&L</b>: $%{y:,.2f}<extra></extra>",
    },
  ...
  createPlot(
    {
      ...
      hovertemplate:
        "<b>Month</b>: %{x}<br><b>P&L</b>: $%{y:,.2f}<extra></extra>",
    },
  ...
  createPlot(
    {
      ...
      hovertemplate:
        "<b>Date</b>: %{x|%Y-%m-%d}<br><b>P&L</b>: $%{y:,.2f}<br><b>Symbol</b>: %{text}<extra></extra>",
    },
  ...
  createPlot(
    {
      ...
      hovertemplate:
        "<b>Date</b>: %{x|%Y-%m-%d}<br><b>ROI</b>: %{y:.1f}%<extra></extra>",
    },
    "Trade ROI %",
    "Date",
    "ROI (%)"
  ...
  createPlot(
    {
      ...
      text: data.map((d) => d.SYMBOL),
      hovertemplate:
        "<b>Date</b>: %{x|%Y-%m-%d}<br><b>Days</b>: %{y:.0f}<br><b>Symbol</b>: %{text}<extra></extra>",
    },
  ...
  const trace = {
    ...
    marker: { color: "#3498db" },
    hovertemplate:
      "<b>P&L Range</b>: $%{x:,.2f}<br><b>Count</b>: %{y}<extra></extra>",
  };
  ...
  createPlot(
    {
      ...
      marker: { color: "#9b59b6" },
      hovertemplate:
        "<b>Holding Period</b>: %{x}<br><b>Win Rate</b>: %{y:.1f}%<extra></extra>",
    },
  ...
  createPlot(
    {
      ...
      hovertemplate:
        "<b>Position Size</b>: $%{x:,.2f}<br><b>ROI</b>: %{y:.1f}%<extra></extra>",
    },
    "Trade Size vs ROI",
    "Position Size ($)",
    "ROI (%)"
...
function calculateHoldingPeriodRanges(data) {
  const sortedPeriods = [...data].sort(
    (a, b) => a.HOLDING_PERIOD - b.HOLDING_PERIOD
  );
  ...
  for (let i = 0; i < numBins; i++) {
    ...
    const winRate =
      (binData.filter((d) => d.TOTAL_GAINLOSS > 0).length / binData.length) *
      100;
...
function createSummaryStats(data) {
  ...
  const stats = {
    ...
    "Win Rate": `${(
      (data.filter((d) => d.TOTAL_GAINLOSS > 0).length / data.length) *
      100
    ).toFixed(1)}%`,
    "Average Win": formatCurrency(
      _.meanBy(
        data.filter((d) => d.TOTAL_GAINLOSS > 0),
        "TOTAL_GAINLOSS"
      )
    ),
    "Average Loss": formatCurrency(
      Math.abs(
        _.meanBy(
          data.filter((d) => d.TOTAL_GAINLOSS < 0),
          "TOTAL_GAINLOSS"
    ...
    Papa.parse(file, {
      ...
      complete: function (results) {
        ...
        try {
          ...
          alert(
            "Error processing data. Please check the CSV format and try again."
          );
        }
      },
